...
int main(int argc, char **argv)
{
	...
	string sub_im_name;
  // pre-load filelist
  vector<string> filelists;   // 从test的列表中导入用来评估的图像
...
#pragma omp parallel for
	for (int i = 0; i < filelists.size(); i++)
	{
		...
		string sub_txt_name =  sub_im_name.substr(0, sub_im_name.find_last_of(".")) + ".lines.txt";    // 将test0_normal里的jpg替换后缀为.lines.txt
		string anno_file_name = anno_dir + sub_txt_name;     // 在culane下面找到对应的图片
		string detect_file_name = detect_dir + sub_txt_name;   // 在predict(culane_eval_tmp)下面找到对应的图片
		vector<vector<Point2f> > anno_lanes;    // ground truth
		vector<vector<Point2f> > detect_lanes;   // predict 生成在cualne_eval_tmp
